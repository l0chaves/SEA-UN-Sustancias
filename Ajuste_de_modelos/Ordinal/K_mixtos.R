...
summary(fitK_mm)
summary(fit3K_mass) #Ajuste de modelos/ordinal/probit/K
...
betaF <- fit3K_mass$coefficients

hat_yF <- hat_X %*% betaF
QpF <- fit3K_mass$zeta #los puntos de corte interceptos estimados
...
pred_mixtos <- as.data.frame(cbind(MD_K$K_04, categoriasM, hat_yM)) 

CM__mixtos <- table(pred_fijos$V1, pred_fijos$categoriasF)
fitted(fitK_mm)
