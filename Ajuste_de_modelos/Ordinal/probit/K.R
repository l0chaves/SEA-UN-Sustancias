...
X <- d %>%
  ...
  dplyr::select(`DIRECTORIO`, `D_01`, `D_02`, `D_06`, `D_07`, `D_08`, `D_10`,
                `D_12_A`, `D_12_B`, `D_12_C`, 
                `D2_01`, `D2_03`, `D2_04`, `D2_05`, `D2_06`, SEXO, TIPO, SERVICIO, ESTRATO,
                `F_12`, `E_04`, `Q_02`, `Q_03`,
                `D_05`,PARENTESCO, EDAD, TOTAL_PERSONAS) %>%
  mutate_at(vars(2:25), as.factor) %>%
  mutate_at(vars(26,27), as.numeric)
...
fitCKc <- polr(factor(K_04) ~ FG_01+G_02+D_11_P+G_11+D_09+CEDAD+SEXO, data = MD_K3, Hess = TRUE, method = "probit")
scope <- list(lower=~FG_01+G_02+D_11_P+G_11+CEDAD+SEXO, 
              upper=~ FG_01 + G_02 + D_11_P + G_11 + D_09 + CEDAD + SEXO +
                D_01 + D_02 + D_06 + D_07 + D_08 + D_10 + +D_12_A + D_12_B + D_12_C + D2_01 + D2_03 + D2_05 + D2_06 + 
                TIPO + ESTRATO + PARENTESCO + F_12 + E_04 + Q_02 + Q_03 + D_05 + TOTAL_PERSONAS + 
...
# Se elimina la variable parentesco por multicolinealidad

fit4K_mass <- polr(factor(K_04) ~ FG_01 + G_02 + D_11_P + G_11 + 
                     D_09 + CEDAD + SEXO + K_12_O + K_10_C + K_12_H + K_12_I + 
                     D2_05 + K_10_D + K_10_E + K_11 + K_12_C + K_10_I+ 
                     D_01 + E_04 + Q_03 + D_12_A + K_12_K,
                   data = MD_K3, Hess = TRUE, method = "probit")

fit4K_ord <- clm(factor(K_04) ~ FG_01 + G_02 + D_11_P + G_11 + 
                   D_09 + CEDAD + SEXO + K_12_O + K_10_C + K_12_H + K_12_I + 
                   D2_05 + K_10_D + K_10_E + K_11 + K_12_C + K_10_I +
                   D_01 + E_04 + Q_03 + D_12_A + K_12_K, data = MD_K3, link = "probit")
...
fit6K_mass <- polr(factor(K_04) ~ FG_01 + G_02 + D_11_P + G_11 + 
                     ...
                     D2_05 + K_10_D + K_10_E + K_11 + K_12_C + K_10_I+ 
                     D_01 + E_04 + Q_03 + D_12_A + K_12_K,
                     data = MD_Kpk, Hess = TRUE, method = "probit",
                     weights = FEX_C, start = c(fit4K_mass$coefficients, fit4K_mass$zeta))
...
fit6K_ord <- clm(factor(K_04) ~ FG_01 + G_02 + D_11_P + G_11 + 
                   ...
                   start = c(fit4K_ord$alpha, fit4K_ord$beta))

summary(fit6K_mass); vif(fit6K_mass)
summary(fit6K_ord)

#Volver a correr con las variables numéricas estandarizadas
MD_Kpk$G_11 <- scale(MD_Kpk$G_11)
...
val_f4 <- validacion(fit_mass = fit4K_mass, fit_ord = fit4K_ord, hat_X = hat_X5, y = MD_Kpk$K_04)
val_f6 <- validacion(fit_mass = fit6K_mass, fit_ord = fit6K_ord, hat_X = hat_X5, y = MD_Kpk$K_04)

fit4K_mass$coefficients/fit6K_mass$coefficients
